...
# ---------------------------------------------------------
st.set_page_config(layout="wide", page_title="Institutional Portfolio System | Pro", page_icon="🏛️", initial_sidebar_state="expanded")
...
def check_password():
    ...
    with col2:
        st.write(""); st.markdown("<h2 style='text-align: center;'>🔒 Accesso Istituzionale</h2>", unsafe_allow_html=True)
...
st.markdown("""
<style>
    .stApp { background-color: #f0f2f6; color: #000000; font-family: 'Roboto', sans-serif; }
    [data-testid="stSidebar"] { background-color: #ffffff; border-right: 1px solid #d1d5db; }
    [data-testid="stSidebar"] * { color: #000000 !important; }
    [data-testid="stSidebar"] .stNumberInput input, [data-testid="stSidebar"] .stSelectbox div[data-baseweb="select"] div { color: #000000 !important; background-color: #f9fafb !important; border-color: #d1d5db !important; }
    ...
    label[data-testid="stMetricLabel"] { color: #4b5563 !important; font-weight: 600 !important; }
    div[data-testid="stMetricValue"] { color: #000000 !important; font-weight: 900 !important; font-size: 2rem !important; }
    [data-testid="stDataFrame"], [data-testid="stTable"] { background-color: #ffffff; border: 1px solid #d1d5db; color: #000000; }
    .stButton > button { background-color: #2563eb; color: #ffffff; border: none; border-radius: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; transition: all 0.3s; }
    ...
    .streamlit-expanderHeader { background-color: #ffffff; color: #000000 !important; border: 1px solid #e5e7eb; font-weight: 600; }
</style>
""", unsafe_allow_html=True)

# ---------------------------------------------------------
# LOGICA DINAMICA E AVANZATA
# ---------------------------------------------------------
...
def auto_classify_assets(asset_names):
    ...
    for asset in asset_names:
        u_asset = asset.upper()
        if any(k in u_asset for k in comm_keywords): groups["COMMODITIES"].append(asset)
        elif any(k in u_asset for k in bond_keywords): groups["BONDS"].append(asset)
        else: groups["EQUITY"].append(asset)
    return groups

STRESS_SCENARIOS = {
    "Lehman 2008": {"start_date": "2008-09-01", "end_date": "2009-02-28", "description": "Crisi finanziaria globale", "shock_multiplier": {"BONDS": 0.9, "EQUITY": 0.5, "COMMODITIES": 0.6}},
    "COVID-19 2020": {"start_date": "2020-02-01", "end_date": "2020-04-30", "description": "Pandemia globale", "shock_multiplier": {"BONDS": 1.1, "EQUITY": 0.6, "COMMODITIES": 0.4}},
    "Inflation Shock 2022": {"start_date": "2022-01-01", "end_date": "2022-10-31", "description": "Picco inflazione e rialzo tassi", "shock_multiplier": {"BONDS": 0.8, "EQUITY": 0.8, "COMMODITIES": 1.2}}
}

class TransactionCostModel:
    def __init__(self, current_aum=100000000):
        self.base_cost = 0.0010
    def estimate_cost(self, turnover_vector, asset_names, market_volatility=0.15):
        ...
        market_impact = turnover_vector * (1/7) * market_volatility * 0.3
        total_cost += market_impact
        return total_cost

class DynamicConfidenceCalculator:
    @staticmethod
    def calculate(ff5_models, returns_data=None, min_conf=0.1, max_conf=1.0):
        confidences = {}
        
        # Calcolo Regime Volatilità (Se returns_data è disponibile)
        vol_regime_adjustment = 0.0
        if returns_data is not None:
            # Volatilità ultimi 3 mesi vs media storica
            recent_vol = returns_data.iloc[-3:].std()
            hist_vol = returns_data.std()
            # Se volatilità recente > 1.2 * storica -> Riduci confidence
            if (recent_vol > hist_vol * 1.2).any(): # Se almeno un asset è molto volatile
                vol_regime_adjustment = -0.15 # Penalità di confidenza
        ...
        for asset, model in ff5_models.items():
            ...
            t_norm = min(1.0, model.tvalues.abs().mean() / 2) if hasattr(model, 'tvalues') else 0.5
            stability_score = 0.7 
            
            # Base confidence
            conf = 0.4 * r2_norm + 0.3 * t_norm + 0.3 * stability_score
            
            # Applica aggiustamento regime volatilità
            conf += vol_regime_adjustment
...
def winsorize_returns(returns, lower_percentile=1, upper_percentile=99):
    ...
    for asset in returns.columns:
        if len(returns[asset].dropna()) > 0:
            lower = np.percentile(returns[asset].dropna(), lower_percentile)
            upper = np.percentile(returns[asset].dropna(), upper_percentile)
            returns_winsorized[asset] = returns[asset].clip(lower, upper)
    return returns_winsorized

def validate_inputs(returns, min_months=24, max_missing=0.3):
    validation_issues = []
    if len(returns) < min_months: validation_issues.append(f"Dati insufficienti: {len(returns)} mesi (< {min_months})")
    missing_pct = returns.isna().sum() / len(returns)
    if (missing_pct > max_missing).any(): validation_issues.append("Asset con troppi dati mancanti.")
...
def get_ff5_data_cutoff(cutoff_date):
    ...
    if ff5_full is not None: return ff5_full.loc[:cutoff_date]
    return None

# --- CORE CALCULATION WITH MOMENTUM ---
def calculate_ff5_views(returns, ff5_data, window=60, return_models=False):
    ...
    # Calcolo Momentum (12-1 Month)
    # Se abbiamo abbastanza dati, calcoliamo il rendimento ultimi 12 mesi escluso l'ultimo
    if len(returns_clean) >= 13:
        momentum_score = (returns_clean.iloc[-2] / returns_clean.iloc[-13]) - 1
    else:
        momentum_score = pd.Series(0, index=returns.columns)
    ...
    for asset in returns.columns:
        try:
            ...
            # Se momentum positivo, aggiungiamo un piccolo bonus (es. +0.5% annualizzato se forte)
            mom_adjustment = 0.0
            asset_mom = momentum_score.get(asset, 0)
            if not np.isnan(asset_mom):
                mom_adjustment = asset_mom * 0.05 # Peso del momentum (5% del segnale)
    ...
    if return_models: return pd.Series(views), pd.DataFrame(betas_dict).T, None, models_dict
    else: return pd.Series(views), pd.DataFrame(betas_dict).T, None
...
def black_litterman_solver(cov_matrix, market_prior, ff5_views, tau=0.05, conf_id=0.5, asset_confidences=None):
    ...
    if np.isscalar(conf_id) and conf_id < 0.01: Omega = np.diag(np.diag(cov_values)) * 1000
    
    inv_tau_cov = np.linalg.inv(tau * cov_values)
    try: inv_omega = np.linalg.inv(Omega)
    except: inv_omega = np.linalg.inv(Omega + np.eye(len(Omega)) * 1e-6)
    ...
    for scenario_name, scenario in scenarios.items():
        try:
            ...
            for group, assets in asset_groups.items():
                ...
                if len(group_cols) > 0: scenario_returns[group_cols] *= multiplier
            ...
            for line_name, weights in weights_dict.items():
                ...
                stress_results.append({'Scenario': scenario_name, 'Linea': line_name, 'Durata (mesi)': len(scenario_returns), 'Rendimento Totale': total_return, 'Max Drawdown': drawdown, 'Descrizione': scenario['description']})
        except: continue
    return pd.DataFrame(stress_results)

def optimize_line(mu, cov, asset_names, target_vol_min=None, target_vol_max=None, risk_free=0.0, min_weight=0.0, max_weight=1.0, group_limits=None, stability_penalty=0.0, previous_weights=None, asset_groups=None):
    num_assets = len(mu)
    ...
    if target_vol_max: constraints.append({'type': 'ineq', 'fun': lambda x: target_vol_max - np.sqrt(np.dot(x.T, np.dot(cov, x))) * np.sqrt(12)})
    if target_vol_min: constraints.append({'type': 'ineq', 'fun': lambda x: np.sqrt(np.dot(x.T, np.dot(cov, x))) * np.sqrt(12) - target_vol_min})
    ...
    if group_limits and asset_groups:
        for group, limit_max in group_limits.items():
            ...
            if indices: constraints.append({'type': 'ineq', 'fun': lambda x, idx=indices, l=limit_max: l - np.sum(x[idx])})
    ...
    try:
        ...
        return result.x
    except: return np.full(num_assets, 1.0/num_assets)
...
def sensitivity_monte_carlo(mu_base, cov_base, asset_names, n_sim=500):
    ...
    if allocations: return pd.Series(np.array(allocations).std(axis=0), index=asset_names)
    return pd.Series(0, index=asset_names)

def run_walk_forward_backtest(returns, ff5_full, vol_ranges, group_limits_base, equity_limits_per_line, min_w, max_w, tx_cost, view_window, conf_level, stability_penalty=0.01, asset_groups=None):
        ...
        if curr_date in rebalance_dates:
            ...
            if ff5_available is not None and len(ff5_available) >= 24:
                ...
                if views_t is not None:
                    # Passiamo i dati storici al calcolatore di confidenza per analizzare il regime di volatilità
                    conf_calc = DynamicConfidenceCalculator()
                    asset_confidences = conf_calc.calculate(models_t, returns_data=past_returns)
                    ...
                    for i, (v_min, v_max) in enumerate(vol_ranges):
                        ...
                        new_w = optimize_line(bl_post_t.values, cov_t.values, returns.columns,
                                            target_vol_min=v_min, target_vol_max=v_max, risk_free=rf_t/12, 
...
if uploaded_file:
    ...
    with col_up_2:
        ...
        with st.expander("🔍 Classificazione Asset (Auto)"):
            c_eq, c_bd, c_cm = st.columns(3)
            c_eq.markdown(f"**Equity ({len(detected_groups['EQUITY'])})**"); c_eq.caption(", ".join(detected_groups['EQUITY']) if detected_groups['EQUITY'] else "-")
            c_bd.markdown(f"**Bonds ({len(detected_groups['BONDS'])})**"); c_bd.caption(", ".join(detected_groups['BONDS']) if detected_groups['BONDS'] else "-")
            c_cm.markdown(f"**Comm ({len(detected_groups['COMMODITIES'])})**"); c_cm.caption(", ".join(detected_groups['COMMODITIES']) if detected_groups['COMMODITIES'] else "-")
    ...
    if ff5_df is not None:
        with st.spinner("Calcolo Asset Allocation Strategica..."):
            ...
            asset_confidences = DynamicConfidenceCalculator.calculate(ff5_models, returns_data=returns_monthly) if use_dynamic_conf and ff5_models else None
            ...
            vol_ranges = [(0.000, 0.025), (0.025, 0.050), (0.050, 0.075), (0.075, 0.100), (0.100, 0.125), (0.125, 0.150)]
            ...
            for name, (v_min, v_max) in zip(line_labels, vol_ranges):
                ...
                w = optimize_line(bl_posterior.values, cov_lw.values, returns_monthly.columns,
                                target_vol_min=v_min, target_vol_max=v_max, risk_free=rf_rate/12, 
