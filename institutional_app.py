...
if uploaded_file:
    ...
    with col_up_2:
        ...
        # Mostra la classificazione rilevata
        with st.expander("🔍 Classificazione Automatica Asset (Modificabile dai nomi colonna)"):
            c_eq, c_bd, c_cm = st.columns(3)
            c_eq.markdown(f"**Equity ({len(detected_groups['EQUITY'])})**")
            c_eq.caption(", ".join(detected_groups['EQUITY']) if detected_groups['EQUITY'] else "Nessuno")
            c_bd.markdown(f"**Bonds ({len(detected_groups['BONDS'])})**")
            c_bd.caption(", ".join(detected_groups['BONDS']) if detected_groups['BONDS'] else "Nessuno")
            c_cm.markdown(f"**Comm ({len(detected_groups['COMMODITIES'])})**")
            c_cm.caption(", ".join(detected_groups['COMMODITIES']) if detected_groups['COMMODITIES'] else "Nessuno")
